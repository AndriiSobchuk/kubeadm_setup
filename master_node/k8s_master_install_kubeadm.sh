...
cd $WD_KUBEADM
touch kubeadm_join.sh full_kubeadm_init_output.sh
...
echo "$KUBEADMIN_OUTPUT" >> full_kubeadm_init_output.sh
tail -n 2 full_kubeadm_init_output.sh > kubeadm_join.sh
